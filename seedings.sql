...
('DAM41UDN3CHU2WVF7', 'Volvo', 'V60 Cross Country', 2019, 'Gray'); # va a fallar por que VIN es PRIMARY KEY Y ESTA DUPLICADO

INSERT INTO customers 
(customer_ID, name, phone_NUMBER, email, address, city, state_province, country, zip_postal_code)
VALUES
...
(
  852399038,
  '2018-08-22',
  (SELECT VIN FROM cars WHERE id = 1),
  (SELECT customer_ID FROM customers WHERE id = 0),
  (SELECT staff_ID FROM salespersons WHERE id = 3)
),
(
  731166526,
  '2018-12-31',
  (SELECT VIN FROM cars WHERE id = 3),
...
(
  271135104,
  '2019-01-22',
  (SELECT VIN FROM cars WHERE id = 2),
